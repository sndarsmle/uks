<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class SMP extends Middleware {

    function __construct()
    {
        parent::__construct();
        $this->load->model('M_Login', 'mLogin');
        $this->load->model('mcu/M_MCU', 'mMCU');
        $this->load->model('M_Uks', 'mUKS');
        $this->load->model('siswa/M_Siswa', 'mSiswa');
    }

    function step1($mcuID){ //vital
        $this->load->helper('jenjang');
        $this->load->model('mcuSMP/M_VitalSMP', 'mVitalSMP');
        $data['role']  = $this->mLogin->getUserRole();
        $mcuData = $this->mMCU->findById($mcuID);
        $vitalData = $this->mVitalSMP->findByParentID($mcuData->mcu_id);

        if($this->input->post()){
            $form = $this->input->post();
            $form['form_id'] = $mcuData->mcu_id;

            if($vitalData){
                if($this->mVitalSMP->updateRow($form)) {
                    redirect("mcu/SMP/step2/".$mcuData->mcu_id);
                }
            }else{
                if($this->mVitalSMP->createRow($form)) {
                    redirect("mcu/SMP/step2/".$mcuData->mcu_id);
                }
            }
        }else{
            $data['title'] = "Step 1";
            $data['menu']  = 'MCU';
            $data['smenu'] = '';
            $data['user']  = $this->mLogin->getNameUser();
            $data['role']  = $this->mLogin->getUserRole();
            $data['siswa'] = $this->mSiswa->findById($mcuData->siswa_id);
            $data['mcu'] = $mcuData;
            $jk                 = $data['siswa']->siswa_kelamin;
            if ($jk=="L") 
            {
                $data ['imtdbb']       = $this->mUKS->getIMTLaki();
            }
             else 
            {
                $data ['imtdbb']       = $this->mUKS->getIMTPerempuan();
            }

            if($vitalData){
                $data['vital'] = $vitalData;
            }else{
                $data['vital'] = (object) [
                    'vital_tekananDarahmm' => '',
                    'vital_tekananDarahhg' => '',
                    'vital_nadi' => '',
                    'vital_freqNafas' => '',
                    'vital_suhu' => '',
                    'vital_bisingJantung' => '',
                    'vital_bisingParu' => '',
                ];
            }
            //var_dump($data['imtdbb']);
            //var_dump($data['siswa']);

            
            $this->blade->render('smp/mcu_step1', $data);
            
        }
    }

    function step2($mcuID){ //gizi
        $this->load->helper('jenjang');
        $this->load->model('mcuSMP/M_GiziSMP', 'mGiziSMP');

        $role = $this->mLogin->getUserRole();
        $mcuData = $this->mMCU->findById($mcuID);
        $giziData = $this->mGiziSMP->findByParentID($mcuData->mcu_id);

        if($this->input->post()){
            $form = $this->input->post();
            $form['form_id'] = $mcuData->mcu_id;

            if($giziData) {
                $this->mGiziSMP->updateRow($form);
            } else {
                $this->mGiziSMP->createRow($form);
            }

            if (isset($form['form_tgl']) && $mcuData) {
                $data['siswa'] = $this->mSiswa->findById($mcuData->siswa_id);
                $born_date = new DateTime($data['siswa']->siswa_tgl_lahir);
                $selected_date = new DateTime($form['form_tgl']);
                $interval_date = $born_date->diff($selected_date);

                $form['form_periode'] = $mcuData->periode_id;
                $form['form_siswa'] = $mcuData->siswa_id;
                $form['form_lokasi'] = $mcuData->mcu_location;
                $form['form_tahun'] = $interval_date->y;
                $form['form_bulan'] = $interval_date->m;
                $form['form_code'] = $mcuData->mcu_code;

                $this->mMCU->updateRow($form);
            }

            if($role == 2) {
                redirect("mcu/reservasi?key=44");
            } else {
                redirect("mcu/SMP/step3/".$mcuData->mcu_id);
            }
        }else{
            $data['title'] = "Step 2";
            $data['menu']  = 'MCU';
            $data['smenu'] = '';
            $data['user']  = $this->mLogin->getNameUser();
            $data['role']  = $this->mLogin->getUserRole();
            $data['siswa'] = $this->mSiswa->findById($mcuData->siswa_id);
            $data['mcu'] = $mcuData;
            $mcuData = $this->mMCU->findById($mcuID);    
            $data['mcu'] = $mcuData;
            $jk                 = $data['siswa']->siswa_kelamin;
            if ($jk=="L") 
            {
                $data ['imtdbb']       = $this->mUKS->getIMTLaki();
            }
             else 
            {
                $data ['imtdbb']       = $this->mUKS->getIMTPerempuan();
            }

            if($giziData){
                $data['gizi'] = $giziData;
            }else{
                $data['gizi'] = (object) [
                    'bb' => '',
                    'tb' => '',
                    'lk' => '',
                    'lla' => '',
                    'lp' => '',
                    'pimt' => '',
                    'status_gizi' => '',
                    'stun' => '',
                    'anemia' => '',
                ];
            }
            //var_dump($data['imtdbb']);
            //var_dump($data['siswa']);
            $this->blade->render('smp/mcu_step2', $data);
        }



        
    }

    function step3($mcuID){ //kebersihan diri
        $this->load->helper('jenjang');
        $this->load->model('mcuSMP/M_BersihSMP', 'mBersihSMP');

        $mcuData = $this->mMCU->findById($mcuID);
        $bersihData = $this->mBersihSMP->findByParentID($mcuData->mcu_id);

        if($this->input->post()){
            $form = $this->input->post();
            $form['form_id'] = $mcuData->mcu_id;

            if($bersihData){
                if($this->mBersihSMP->updateRow($form)) {
                    redirect("mcu/SMP/step4/".$mcuData->mcu_id);
                }
            }else{
                if($this->mBersihSMP->createRow($form)) {
                    redirect("mcu/SMP/step4/".$mcuData->mcu_id);
                }
            }
        }else{
            $data['title'] = "Step 3";
            $data['menu']  = 'MCU';
            $data['smenu'] = '';
            $data['user']  = $this->mLogin->getNameUser();
            $data['role']  = $this->mLogin->getUserRole();
            $data['siswa'] = $this->mSiswa->findById($mcuData->siswa_id);
            $data['mcu'] = $mcuData;
            $mcuData = $this->mMCU->findById($mcuID);    
            $data['mcu'] = $mcuData;


            if($bersihData){
                $data['bersih'] = $bersihData;
            }else{
                $data['bersih'] = (object) [
                    'rambut' => '',
                    'kulit' => '',
                    'ket_kulit' => '',
                    'kulit_sisik' => '',
                    'kulit_memar' => '',
                    'kulit_sayat' => '',
                    'kulit_koreng' => '',
                    'kulit_koreng_sukar' => '',
                    'kulit_suntik' => '',
                    'kuku' => '',
                    
                ];
            }
            //var_dump($data['imtdbb']);
            //var_dump($data['siswa']);
            $this->blade->render('smp/mcu_step3', $data);
        }

        // $data['title'] = "Step 3";
        // $data['menu']  = 'MCU';
        // $data['smenu'] = '';
        // $data['user']  = $this->mLogin->getNameUser();
        // $data['role']  = $this->mLogin->getUserRole();
        // $data['siswa'] = $this->mSiswa->findById($mcuData->siswa_id);
        // $data['mcu'] = $mcuData;

        // $this->blade->render('smp/mcu_step3', $data);
    }
    function step4($mcuID){ //kesehatan mulut
        $this->load->helper('jenjang');
        $this->load->model('mcuSMP/M_MulutSMP', 'mMulut');

        $mcuData = $this->mMCU->findById($mcuID);
        $mulutData = $this->mMulut->findByParentID($mcuData->mcu_id);

        if($this->input->post()){
            $form = $this->input->post();
            $form['form_id'] = $mcuData->mcu_id;

            if($mulutData){
                if($this->mMulut->updateRow($form)) {
                    redirect("mcu/SMP/step5/".$mcuData->mcu_id);
                }
            }else{
                if($this->mMulut->createRow($form)) {
                    redirect("mcu/SMP/step5/".$mcuData->mcu_id);
                }
            }
        }else{
            $data['title'] = "Step 4";
            $data['menu']  = 'MCU';
            $data['smenu'] = '';
            $data['user']  = $this->mLogin->getNameUser();
            $data['role']  = $this->mLogin->getUserRole();
            $data['siswa'] = $this->mSiswa->findById($mcuData->siswa_id);
            //$data['mcu'] = $mcuData;
            $mcuData = $this->mMCU->findById($mcuID);    
            $data['mcu'] = $mcuData;


            if($mulutData){
                $data['mulut'] = $mulutData;
            }else{
                $data['mulut'] = (object) [
                    'bibir' => '',
                    'sudut_mulut' => '',
                    'sariawan' => '',
                    'lidah' => '',
                    'luka_lain' => '',
                    'ket_masalah_lain_rongga_mulut' => '',
                    'caries' => '',
                    'gigi_mud_berdarah' => '',
                    'gusi_bengkak' => '',
                    'gigi_kotor' => '',
                    'karang_gigi' => '',
                    'gigi_dep' => '',
                    'ket_masalah_lain_gigi_gusi' => '',
                    
                    
                ];
            }
            //var_dump($data['imtdbb']);
            //var_dump($data['siswa']);
            $this->blade->render('smp/mcu_step4', $data);
        }
        // $this->load->helper('jenjang');
        // $mcuData = $this->mMCU->findById($mcuID);

        // $data['title'] = "Step 4";
        // $data['menu']  = 'MCU';
        // $data['smenu'] = '';
        // $data['user']  = $this->mLogin->getNameUser();
        // $data['role']  = $this->mLogin->getUserRole();
        // $data['siswa'] = $this->mSiswa->findById($mcuData->siswa_id);
        // $data['mcu'] = $mcuData;

        // $this->blade->render('smp/mcu_step4', $data);
    }
    function step5 ($mcuID){ //Penglihatan dan Pendengaran
        $this->load->helper('jenjang');
        $this->load->model('mcuSMP/M_MatatelingaSMP', 'mMata');

        $mcuData = $this->mMCU->findById($mcuID);
        $mataData = $this->mMata->findByParentID($mcuData->mcu_id);

        if($this->input->post()){
            $form = $this->input->post();
            $form['form_id'] = $mcuData->mcu_id;

            if($mataData){
                if($this->mMata->updateRow($form)) {
                    redirect("mcu/SMP/step6/".$mcuData->mcu_id);
                }
            }else{
                if($this->mMata->createRow($form)) {
                   
                    redirect("mcu/SMP/step6/".$mcuData->mcu_id);
                }
            }
        }else{
            $data['title'] = "Step 5";
            $data['menu']  = 'MCU';
            $data['smenu'] = '';
            $data['user']  = $this->mLogin->getNameUser();
            $data['role']  = $this->mLogin->getUserRole();
            $data['siswa'] = $this->mSiswa->findById($mcuData->siswa_id);
            //$data['mcu'] = $mcuData;
            $mcuData = $this->mMCU->findById($mcuID);    
            $data['mcu'] = $mcuData;


            if($mataData){
                $data['matatelinga'] = $mataData;
            }else{
                $data['matatelinga'] = (object) [
                    'mata_luar' => '',
                    'penglihatan' => '',
                    'ket_penglihatan' => '',
                    'buta_warna' => '',
                    'inf_mata' => '',
                    'telinga' => '',
                    'kot_telinga' => '',
                    'inf_telinga' => '',
                    'ket_masalah_lain_pendengaran' => '',
                ];
            }

            $this->blade->render('smp/mcu_step5', $data);
            //var_dump($data);
        }
        
    }
    function step6 ($mcuID){ //Lainnyaa
        $this->load->helper('jenjang');
        $this->load->model('mcuSMP/M_LainSMP', 'mLain');

        $role = $this->mLogin->getUserRole();
        $mcuData = $this->mMCU->findById($mcuID);
        $lainData = $this->mLain->findByParentID($mcuData->mcu_id);

        if($this->input->post()){
            $form = $this->input->post();
            $form['form_id'] = $mcuData->mcu_id;

            if($lainData){
                if($this->mLain->updateRow($form)) {
                    if($role == 3){
                        redirect("mcu/SMP/evaluasi/".$mcuData->mcu_id);
                    }else{
                        redirect("mcu/SMP/step6/".$mcuData->mcu_id);
                    }
                }
            }else{
                if($this->mLain->createRow($form)) {
                    if($role == 3){
                        redirect("mcu/SMP/evaluasi/".$mcuData->mcu_id);
                    }else{
                        redirect("mcu/SMP/step6/".$mcuData->mcu_id);
                    }
                }
            }
        }else{
            $data['title'] = "Step 6";
            $data['menu']  = 'MCU';
            $data['smenu'] = '';
            $data['user']  = $this->mLogin->getNameUser();
            $data['role']  = $this->mLogin->getUserRole();
            $data['siswa'] = $this->mSiswa->findById($mcuData->siswa_id);
            //$data['mcu'] = $mcuData;
            $mcuData = $this->mMCU->findById($mcuID);    
            $data['mcu'] = $mcuData;


            if($lainData){
                $data['lain'] = $lainData;
            }else{
                $data['lain'] = (object) [
                    'mental' => '',
                    'saran' => '',
                    'kesimpulan' => '',
                    'followup' => '',
                                      
                    
                ];
            }

            $this->blade->render('smp/mcu_step6', $data);
            //var_dump($data);
        }


        
    }

    function evaluasi($mcuID)
    {
        $this->load->helper('jenjang');
        $this->load->helper('date');
        $mcuData = $this->mMCU->findByIdAllDataSMP($mcuID);
        //$lainData = $this->mLain->findByParentID($mcuData->mcu_id);
        if($this->input->post()){
            // $form = $this->input->post();
            $form['form_id'] = $mcuData->mcu_id;
            $form['form_admin'] = $this->mLogin->getUserID();
            {
                $this->mMCU->UpdateStatusFinish($form);
                //$this->mLain->updateRow($form); 
                redirect("mcu");  
            }
        }else{
            $data['title'] = "Evaluasi";
            $data['menu']  = 'MCU';
            $data['smenu'] = '';
            $data['user']  = $this->mLogin->getNameUser();
            $data['role']  = $this->mLogin->getUserRole();
            $data['siswa'] = $this->mSiswa->findById($mcuData->siswa_id);
            $data['mcu'] = $mcuData;
            $this->blade->render('smp/mcu_evaluasi', $data);
        }     
    }
} 