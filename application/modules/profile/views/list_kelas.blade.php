                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive" >
                        <table class="table table-bordered table-hover" id="table">
                            ...
                            <tbody>
                            @foreach($list_kelas as $i => $data)
                                <tr>
                                    <td><b>{{ $i+=1 }}</b></td>
                                    <td>{{ formatJenjang($data->siswa_jenjang) }} - {{ $data->kelas_tingkat }}{{ $data->kelas_rombel }}</td>
                                    <td>
                                        <a href="{{ base_url('profile/list_siswa/'.$data->siswa_jenjang.'/'.$data->kelas_tingkat.'/'.$data->kelas_rombel) }}" class="btn btn-success" style="color:white">Lihat</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
